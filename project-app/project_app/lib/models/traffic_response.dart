...
class Leg {
  ...
  factory Leg.fromJson(Map<String, dynamic> json) => Leg(
    viaWaypoints: json["via_waypoints"] != null 
        ...
        : [], // Manejar el caso donde via_waypoints es null
    admins: json["admins"] != null 
        ? List<Admin>.from(json["admins"].map((x) => Admin.fromJson(x))) 
        : [], // Manejar el caso donde admins es null
    weight: json["weight"]?.toDouble() ?? 0.0, // Valor por defecto en caso de null
    duration: json["duration"]?.toDouble() ?? 0.0, // Valor por defecto en caso de null
    steps: json["steps"] != null 
        ? List<dynamic>.from(json["steps"].map((x) => x)) 
        : [], // Manejar el caso donde steps es null
    distance: json["distance"]?.toDouble() ?? 0.0, // Valor por defecto en caso de null
    summary: json["summary"] ?? '', // Manejar el caso donde summary es null
);
